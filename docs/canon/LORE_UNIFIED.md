...
**Legacy source lineage:** legacy:risk_driven_design_bible, legacy:final_narrative_structure  
...
Il Registro non osserva risultati, ma pattern di accettazione.
...
8. LA FINE DEL CIRCUITO

Quando il Registro determina che:
...
Il Registro non dovrebbe fallire.
...
- non parla
- non viene raccontato direttamente
...
Felix viene citato solo dopo che il Registro mostra la prima imperfezione.
...
- Ha accettato scommesse valide
- Ha accumulato Scar legittime
...
5. COSA NON SAPPIAMO (INTENZIONALE)

Il Registro non conserva (o non espone):

- il finale di Felix
- la sua uscita dal sistema
- il suo destino
...
Il player non sta ripercorrendo la strada di Felix.
...
Solo in fasi avanzate del progetto, forse:

- frammenti contraddittori
- log corrotti
...
10. CONDIZIONE DI FALLIMENTO DEL FILE

Questo file fallisce se:
...
**Felix Gallicus non ha rotto il Registro.
...
- nessuna sentenza viene letta

La run termina senza lettura.
...
- apparire nei menu
- apparire nei finali
- apparire come stato noto

Statisticamente:

meno di una run ogni decine di migliaia.

Anche questo numero non deve mai essere comunicato.
...
- pattern di rischio coerente ma non convergente
...
### Cosa succede al posto del finale

- nessun testo del Registro
- nessuna lettura
- nessuna parola conclusiva

Il flow:

- si interrompe
- ritorna al menu
- senza transizione narrativa
...
Nemmeno dal Registro.

## 6. PERCEZIONE DEL PLAYER

Il player non deve essere sicuro che:

- sia successo qualcosa di unico
- sia un bug
- sia un evento reale
...
- alcuni log suggeriscono interruzioni analoghe
- precedenti incompleti
- classificazioni mancanti

Il gioco non conferma mai.
...
Il Silenzio non deve mai:

- essere spiegato
- essere patchato come bug
- essere citato nei file di testo
- diventare contenuto
...
## 2. CONDIZIONE DI INNESCO

La corruzione non si attiva:

- per numero di run
- per avanzamento temporale
- per sblocco diretto

Si attiva quando co-esistono:

- precedenti anomali (Felix Gallicus)
- run che sfiorano o replicano tali pattern
...
## 4. COSA NON CAMBIA MAI (INVARIANTI)

Anche corrotto, il Registro:
...
Tutte le corruzioni successive:

fanno riferimento implicito a lui  
senza mai spiegare cosa sia successo.

## 6. IMPATTO SUL GAME FLOW

La corruzione non cambia il gameplay diretto.
...
La corruzione accade, non si cerca.
...
**Legacy source lineage:** legacy:risk_driven_design_bible, legacy:the_register  
**Last updated:** 2026-02-11  
**Notes:** Overlaps with: legacy:felix_gallicus, legacy:registry_corruption, legacy:registry_silence.
...
- la run rientra in stato di anomalia
- o sfiora un precedente (Felix Gallicus)
...
- tutte e 4 le sezioni presenti
- classificazione chiusa
- Registro coerente
...
- frattura presente
- classificazione incompleta o sospesa
- riferimento implicito a precedenti
