...
def _png_header(path: Path) -> tuple[int, int, int]:
    raw = path.read_bytes()
    if raw[:8] != b"\x89PNG\r\n\x1a\n" or raw[12:16] != b"IHDR":
        raise AssertionError("arena threshold texture must be a PNG with an IHDR header")
    width, height, bit_depth, color_type = struct.unpack(">IIBB", raw[16:26])
    if bit_depth != 8:
        raise AssertionError(f"arena threshold PNG must use 8-bit channels, got {bit_depth}")
...
def _assert_texture() -> None:
    if not TEXTURE.exists():
        raise AssertionError("missing arena_threshold_base.png")
    width, height, color_type = _png_header(TEXTURE)
    if color_type != 6:
        raise AssertionError(f"arena threshold PNG must be RGBA, got PNG color type {color_type}")
    if height <= 0 or not math.isclose(width / height, 2.5, rel_tol=0.002):
        raise AssertionError(f"arena threshold PNG must be 5:2, got {width}x{height}")


def _assert_shared_style_geometry() -> None:
    geometry: tuple[str, ...] | None = None
    for state in STYLE_NAMES:
        path = STYLE_DIR / f"sb_arena_threshold_{state}.tres"
        text = _read(path)
        if TEXTURE_RES_PATH not in text:
            raise AssertionError(f"{path.name} must share arena_threshold_base.png")
        values = tuple(
            re.findall(
        ...
        if geometry is None:
            ...
            raise AssertionError(f"{path.name} changes arena-threshold geometry between states")
